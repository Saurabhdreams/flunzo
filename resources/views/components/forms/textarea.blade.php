...
<div>
    ...
    {{-- Label --}}
    @if ($label)
        <label for="textarea-input-component-id-{{ $model }}" class="block text-sm font-medium {{ $errors->first($model) ? 'text-red-600 dark:text-red-500' : 'text-gray-600 dark:text-white' }}">{!! $label !!}</label>
    @endif
